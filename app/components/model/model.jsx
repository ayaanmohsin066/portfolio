...
export const Model = ({
  ...
  const handlePointerDown = useCallback(
    ...
    [reduceMotion, rotationX, rotationY]
  );

  const handlePointerMove = useCallback(
    event => {
      if (!isDraggingRef.current) return;
      const { x, y, rotationX: startRX, rotationY: startRY } = dragStartRef.current;
      const deltaX = (event.clientX - x) * 0.008;
      const deltaY = (event.clientY - y) * -0.008;
      rotationY.set(startRY + deltaX);
      rotationX.set(startRX + deltaY);
    },
    [rotationX, rotationY]
  );
  ...
  // Handle mouse move animation (only when not dragging)
  useEffect(() => {
...
const Device = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const isCustomCanvas = typeof texture.getCanvas === 'function';
      let placeholder;

      if (isCustomCanvas) {
        const canvas = texture.getCanvas();
        placeholder = new CanvasTexture(canvas);
        placeholder.anisotropy = renderer.current.capabilities.getMaxAnisotropy();
      }

      const [placeholderTexture, gltf] = await Promise.all([
        isCustomCanvas
      ...
      gltf.scene.traverse(async node => {
        ...
        if (node.name === MeshType.Screen) {
          ...
          applyScreenTexture(placeholderTexture, placeholderScreen.current);

          loadFullResTexture = async () => {
            if (isCustomCanvas) {
              await applyScreenTexture(placeholderTexture, node);
            } else {
